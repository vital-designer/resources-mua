 ...
 ******************************************************************************/
	...
	$comment_desc['radio'] 		= 'Multiple Choixe';
	...
	if($is_new_form){ //this is a new form, insert new form data
		...
		if(is_writable(DATA_DIR)){
			...
			mkdir(DATA_DIR."/form_{$form_id}/files",0777);
	...
	function table_add_field($form_id,$element_id,$type,$option_id=0){
		...
		if(('text' == $type) || ('phone' == $type) || ('simple_phone' == $type) || ('url' == $type) || ('email' == $type) || ('file' == $type)){
			...
			$query = "ALTER TABLE `ap_form_{$form_id}` ADD COLUMN `element_{$element_id}` tinyint(6) unsigned NOT NULL DEFAULT '0' COMMENT '{$comment}';";
			do_query($query);
		}elseif ('time' == $type){
			...
			$query = "ALTER TABLE `ap_form_{$form_id}` ADD COLUMN `element_{$element_id}_{$option_id}` tinyint(6) unsigned NOT NULL DEFAULT '0' COMMENT '{$comment} - {$option_id}';";
			do_query($query);
		}
