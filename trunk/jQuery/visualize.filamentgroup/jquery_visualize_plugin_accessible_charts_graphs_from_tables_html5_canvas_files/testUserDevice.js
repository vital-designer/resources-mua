...
/*
 * testuserdevice.js - capabilities testing library
 * Authored by Scott Jehl, Filament Group (filamentgroup.com)
 * Date: July 2007 (latest rev. April 2009)
...
var testuserdevice = function(settings){
	//for referencing
	var that = this;
	
	//config
	that.settings = {
		testName: 'enhancements', //sets cookie name for test case and className on documentElement
		loadScripts: [], //array of js file urls to load
		loadStyles: [], //array of css file urls to load. Can optionally be objects with url and media props
		queueLoading: true, //load scripts consecutively to safeguard dependencies
		appendTo: 'head', //where to append scripts and styles
		appendToggleLink: true,
		forcePassText: 'View High-bandwidth version',
		forceFailText: 'View Low-bandwidth version',
		tests: {  //tests hash - each method should return true/false
			getById: function(){
	...
	//public methods - used in pass/fail links but also
	//can be used for custom pass/fail toggles
	
	//reload page with enhancements disabled
	...
	that._runtests = function(){
		...
		if(cookieGrade){
			...
			if(cookieGrade == 'pass'){
				...
				if(settings.log){console.log(cookieGrade);}
			}	
		...
		//no cookies - run tests
			...
			that._bodyOnReady(function(){
				for(var value in settings.tests){
					if(!settings.tests[value]()){testResult = 'fail';}
	...
	that._enhancePage = function(){
		...
		//append styles
		var h = document.getElementsByTagName(settings.appendTo)[0];
		//first styles
