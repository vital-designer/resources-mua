	...
	function process_form_update($input){
		...
		if(!empty($uploaded_files)){
			...
			foreach ($uploaded_files as $element_name){
				...
				$destination_file = DATA_DIR."/form_{$form_id}/files/{$element_name}-{$entry_id}-{$_FILES[$element_name]['name']}";
				if (move_uploaded_file($_FILES[$element_name]['tmp_name'], $destination_file)) {
					$filename = mysql_real_escape_string($_FILES[$element_name]['name']);
					$query = "update ap_form_{$form_id} set $element_name='{$element_name}-{$entry_id}-{$filename}' where id='$entry_id'";
					do_query($query);
				
					//remove old file if any
					if(!empty($old_filename)){
						@unlink(DATA_DIR."/form_{$form_id}/files/".$old_filename);
					}
				}
			}
		}
	...
	function commit_form_review($form_id,$record_id,$options=array()){
		...
		if(!empty($file_uploads_array)){
			...
			foreach ($file_uploads_array as $element_name){
				...
				$target_filename 	  = $options['machform_data_path'].DATA_DIR."/form_{$form_id}/files/{$filename}.tmp";
				
				$regex    = '/^element_([0-9]*)-([0-9]*)-(.*)$/';
				$matches  = array();
				preg_match($regex, $filename,$matches);
				$filename_noelement = $matches[3];
				
				$destination_filename = $options['machform_data_path'].DATA_DIR."/form_{$form_id}/files/{$element_name}-{$new_record_id}-{$filename_noelement}";
				...
				//build update query
				$file_update_query .= "`{$element_name}`='{$element_name}-{$new_record_id}-{$filename_noelement}',";
			}
