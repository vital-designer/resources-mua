...
// namespace encapsulated variables
var 
...
function dateFormat(format, timestamp, capitalize) {
	...
	var date = new Date(timestamp * timeFactor),
		...
		replacements = {
			...
			'I': pad(hours % 12), // Two digits hours in 12h format, 00 through 11
			'l': hours % 12, // Hours in 12h format, 1 through 11
		...
		each(series, function(serie) {
			...
			if (serie.needsAxes) { // line, column etc. need axes, pie doesn't
				...
				each(serie.data, function(point, i) {
					...
					if (yMin === undefined) yMin = yMax = point[1];
					...
					if (stacking) chart.stacks[serie.type][point[0]] = { total: stackedPoint };
				});
			}
	...
	function Axis (chart, options, horiz) {
		...
		function addPlotLine(pos, color, width) {
			...
			if (width) {
				...
				backgroundLayer.drawLine(x1, y1, x2, y2, color, width);
		...
		function addTick(pos, tickPos, color, width, len, withLabel) {
			...
			if (width) backgroundLayer.drawLine(x1, y1, x2, y2, color, width);
	...
	function MouseTracker (chart, options) {
		...
		function createImageMap () {
			...
			img = createElement('img', {
				...
				// image for the image map to work.
				src: 'data:image/gif;base64,R0lGODlhAQABAJH/AP///0////'
			}, {
				width: plotWidth + PX,
				...
				border: 'none',
				position: ABSOLUTE,
				// Workaround: if not clipped, the y axis will flicker in 
...
LineSeries.prototype = {
		...
		each(this.data, function(pt){
			...
			if (stacking) {
				...
				yBottom = pointStack.cum = 
					(pointStack.cum || pointStack.total) - yValue; // start from top
	...
	drawPointState: function(point, state){
		...
		if (state) {
			...
			if (mOptions) 
				this.drawMarker(singlePointLayer, inverted ? chart.plotWidth - point.y : point.x, inverted ? chart.plotHeight - point.x : point.y, mOptions);
		}
	},
...
var SplineSeries = extendClass( LineSeries, {
	 ...
	 */
	drawLine: function(state) {
		this.segments = this.splinedata || this.getSplineData();
		LineSeries.prototype.drawLine.apply(this, arguments);	
	},
	/**
...
var ColumnSeries = extendClass(LineSeries, {
	...
	translate: function() {
		...
		var options = this.options,
			chart = this.chart,
			categoryWidth = this.data[1].x - this.data[0].x,
			...
			columnNumber = chart.options.xAxis.reversed ? chart.columnCount - this.columnNumber : this.columnNumber - 1,
			pointX = -(categoryWidth / 2) + groupPadding + columnNumber *
				pointOffsetWidth + pointPadding,
			pointY0 = chart.plotWidth - chart.xAxis.translate(0);
				
		// record the new values
		each (this.data, function(point) {
			point.x += pointX;
			point.w = pointWidth;
			point.y0 = chart[chart.inverted ? 'plotWidth' : 'plotHeight']  - chart.yAxis.translate(0);
	...
	drawPoints: function(state) {
		...
		each (data, function(point) {
			...
			if (point.y !== undefined) layer.drawRect(
				inverted ? point.y0 - chart.plotWidth : point.x,
	...
	getAreaCoords: function() {
		var areas = [],
			...
			inverted = chart.inverted;
		each (this.data, function(point) {
			var x1 = inverted ? point.y0 - chart.plotWidth : point.x,
