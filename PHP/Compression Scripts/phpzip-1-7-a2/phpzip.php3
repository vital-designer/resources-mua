...
// --------------------------------------------------------------------------------
// Français :
//   La description de l'installation et des fonctions de PhpZip 1.6 peut être
...
//   * Auteur :
//
//   Ce code a été écrit par Vincent Blavet (vincent@blavet.net) sur ses temps de
//   loisir.
//
// --------------------------------------------------------------------------------
// CVS : $Id: phpzip.php3,v 1.4 2001/12/27 15:49:14 vblavet Exp $
// --------------------------------------------------------------------------------

  // ----- Configuration directory (without '/' at the end)
  $g_config_dir = "config";

  // ----- Extract current script file
  $g_script_name = basename($PATH_TRANSLATED);

  // ----- Extract extension type (.php3/.php/...)
  $g_extension = substr(strrchr($g_script_name, '.'), 1);

  // ----- Configuration file name
  $g_config_file = $g_config_dir."/phpzip.cfg.".$g_extension;
  ...
  if (!is_file($g_lib_dir."/pcltrace.lib.".$g_extension))
  {
    // ----- Error message
    die("<div align=center><font size=4 color=red>Unable to find library '$g_lib_dir/pcltrace.lib.$g_extension' in file '".__FILE__."', line ".__LINE__."</font></div>");
  }

  // ----- Include Library
  include ($g_lib_dir."/pcltrace.lib.".$g_extension);

  // ----- Check the PhpZip Library
  if (!is_file($g_lib_dir."/phpzip.lib.".$g_extension))
  {
    // ----- Error message
    die("<div align=center><font size=4 color=red>Unable to find library '$g_lib_dir/phpzip.lib.$g_extension' in file '".__FILE__."', line ".__LINE__."</font></div>");
  }

  // ----- Include Library
  include ($g_lib_dir."/phpzip.lib.".$g_extension);

  // ----- Check the PclTar Library
  if (!is_file($g_lib_dir."/pcltar.lib.".$g_extension))
  {
    // ----- Error message
    die("<div align=center><font size=4 color=red>Unable to find library '$g_lib_dir/pcltar.lib.$g_extension' in file '".__FILE__."', line ".__LINE__."</font></div>");
  }

  // ----- Include Library
  $g_pcltar_lib_dir = $g_lib_dir;
  include ($g_lib_dir."/pcltar.lib.".$g_extension);

  // ----- Modification of phpzipunzip path
  $g_phpzip_autounzip = $g_lib_dir."/phpunzip.lib.".$g_extension;
  ...
  if (!is_file($g_lib_dir."/pclzip.lib.".$g_extension))
  {
    // ----- Error message
    die("<div align=center><font size=4 color=red>Unable to find library '$g_lib_dir/pclzip.lib.$g_extension' in file '".__FILE__."', line ".__LINE__."</font></div>");
  }

  // ----- Include Library
  include ($g_lib_dir."/pclzip.lib.".$g_extension);

  // ----- Include language file
  if (!is_file("$g_language_dir/lang-".$g_language.".inc.php3"))
  {
    function Translate($text) { return $text; }
  }
  else
  {
    include ("$g_language_dir/lang-".$g_language.".inc.php3");
  }

  // ----- Global variables
  $g_phpzip_app_version = "1.7-alpha";
  ...
  // ----- Module inclusion
  include ('phpzip-zip.'.$g_extension);
...
?>
<script language="JavaScript1.2" src="script/pcsexplorer.js"></script>
...
<?
  ...
  echo "menu1[0]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=new>".Translate("Nouveau")." ...</a></font><br>'\n";
  global $PATH_INFO;
  echo "menu1[1]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=read onClick=\'PcjsExplorer(\"target_type=opener\", \"target=$v_url?a_action=list\", \"target_argument_name=a_archive\", \"type=file\", \"calling_dir=".dirname($PATH_INFO)."\", \"start_dir=.\", \"filter_extension=tar tgz gz piz zip\");return false;\'>".Translate("Ouvrir")." ...</a></font><br>'\n";
  if ($p_archive=="")
  {
    echo "menu1[2]='<font face=$g_font size=$g_title_size color=$g_title_color><i>".Translate("Détruire")."</i></font><br>'\n";
    echo "menu1[3]='<font face=$g_font size=$g_title_size color=$g_title_color><i>".Translate("Télécharger")."</i></font><br>'\n";
  }
  else
  {
    echo "menu1[2]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=erase&a_archive=$p_archive>".Translate("Détruire")."</a></font><br>'\n";
    echo "menu1[3]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=download&a_archive=$p_archive target=_blank>".Translate("Télécharger")."</a></font><br>'\n";
  }
  echo "menu1[4]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=new>".Translate("Quitter")."</a></font><br>'\n";

  //Contents for menu 2
  echo "var menu2=new Array()\n";
  if ($p_archive=="")
  {
    echo "menu2[0]='<font face=$g_font size=$g_title_size color=$g_title_color><i>".Translate("Lister")."</i></font><br>'\n";
    echo "menu2[1]='<font face=$g_font size=$g_title_size color=$g_title_color><i>".Translate("Ajouter")." ...</i></font><br>'\n";
    echo "menu2[2]='<font face=$g_font size=$g_title_size color=$g_title_color><i>".Translate("Mettre à jour")." ...</i></font><br>'\n";
    echo "menu2[3]='<font face=$g_font size=$g_title_size color=$g_title_color><i>".Translate("Extraire")." ...</i></font><br>'\n";
    echo "menu2[4]='<font face=$g_font size=$g_title_size color=$g_title_color><i>".Translate("Supprimer fichiers")." ...</i></font><br>'\n";
  }
  else
  {
    echo "menu2[0]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=list&a_archive=$p_archive>".Translate("Lister")."</a></font><br>'\n";
    echo "menu2[1]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=add&a_archive=$p_archive>".Translate("Ajouter")." ...</a></font><br>'\n";
    if (AppPhpzipArchiveType($p_archive) != 'zip')
    {
      if ($v_archive_type == "phpzip")
        echo "menu2[2]='<font face=$g_font size=$g_title_size color=$g_title_color><i>".Translate("Mettre à jour")." ...</i></font><br>'\n";
      else
        echo "menu2[2]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=update&a_archive=$p_archive>".Translate("Mettre à jour")." ...</a></font><br>'\n";
      echo "menu2[3]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=unzip&a_archive=$p_archive>".Translate("Extraire")." ...</a></font><br>'\n";
      if ($v_archive_type == "phpzip")
        echo "menu2[4]='<font face=$g_font size=$g_title_size color=$g_title_color><i>".Translate("Supprimer fichiers")." ...</i></font><br>'\n";
      else
        echo "menu2[4]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=del_file&a_archive=$p_archive>".Translate("Supprimer fichiers")." ...</a></font><br>'\n";
    }
  }

  //Contents for menu 3
  echo "var menu3=new Array()\n";
  echo "menu3[0]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=option&a_archive=$p_archive>".Translate("Options")."</a></font><br>'\n";

  //Contents for menu 4
  echo "var menu4=new Array()\n";
  echo "menu4[0]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=help&a_archive=$p_archive&a_topic=$v_help_ext>".Translate("Aide")."</a></font><br>'\n";
  echo "menu4[1]='<font face=$g_font size=$g_title_size><a class=head href=$v_url?a_action=about>".Translate("Au sujet de")." ...</a></font><br>'\n";
?>
</script>
...
function PcjsZipFileOpenPopup(index, name, archive, is_dir)
{
  ...
  // ----- Update the links
  document.all.link_extract.href="<? echo $v_url; ?>?a_action=extract_index&a_archive="+archive+"&a_index="+index;
  document.all.link_extract_plus.href="<? echo $v_url;  ?>?a_action=extract_index_ask&a_archive="+archive+"&a_index="+index;
  document.all.link_delete.href="<? echo $v_url; ?>?a_action=del_file_do&a_archive="+archive+"&a_file[index]="+name+"&a_index="+index;
<?php
  if (AppPhpzipArchiveType($p_archive)!='zip')
  {
?>
  document.all.link_update.href="<?php echo $v_url; ?>?a_action=update_index&a_archive="+archive+"&a_file[index]="+name;
<?php
  }
?>
...
function PcjsZipFileConfirmExtract()
{
  ...
  {
    alert('<? echo Translate("Aucun fichier sélectionné.")."\\n".Translate("Extration abandonnée."); ?>');
    return false;
  }
  else
  {
    return confirm("<? echo Translate("Confirmer l'extraction de"); ?> '"+PcjsZipSelectedFile+"'");
  }
}
...
function PcjsZipFileConfirmDelete()
{
  ...
  {
    return confirm("<? echo Translate("Confirmer la suppression de"); ?> '"+PcjsZipSelectedFile+"'");
  }
}
...
function PcjsZipFileConfirmUpdate()
{
  ...
  {
    return confirm("<? echo Translate("Confirmer la mise à jour de"); ?> '"+PcjsZipSelectedFile+"'");
  }
}
...
function PcjsZipFileGeneratePopup()
{
  ...
  document.write("<div id=PcjsZipFile style='position:absolute; left:118px; top:214px; z-index:100; visibility:hidden; background-color: <? echo $g_title_color; ?>; layer-background-color: <? echo $g_title_color; ?>; border: 1px none #000000'> ");
  document.write("<table border=0 cellspacing=0 bgcolor=<? echo $g_title_bg; ?>>");
  document.write("<tr height=5><td colspan=3></td></tr>");
  document.write("<tr><td width=2></td><td>");
  document.write('<? echo "<font face=$g_font size=$g_title_size><a id=link_extract class=head href=$v_url?a_action=list&a_archive=$p_archive onClick=\\'if (PcjsZipFileConfirmExtract()) { PcjsActionWindow(document.all.link_extract.href,\"\"); } return false;\\'>".Translate("Extraire")."</a></font><br>";?>');
  document.write('<? echo "<font face=$g_font size=$g_title_size><a id=link_extract_plus class=head href=$v_url?a_action=list&a_archive=$p_archive onClick=\\'PcjsActionWindow(document.all.link_extract_plus.href,\"\"); return false;\\'>".Translate("Extraire")." ...</a></font><br>"; ?>');
  document.write('<? echo "<font face=$g_font size=$g_title_size><a id=link_delete class=head href=$v_url?a_action=list&a_archive=$p_archive onClick=\\'if (PcjsZipFileConfirmDelete()) { PcjsActionWindow(document.all.link_delete.href,\"\"); } return false;\\'>".Translate("Supprimer")."</a></font><br>"; ?>');
<?php
  if (AppPhpzipArchiveType($p_archive)!='zip')
  {
?>
  document.write('<? echo "<font face=$g_font size=$g_title_size><a id=link_update class=head href=$v_url?a_action=list&a_archive=$p_archive onClick=\\'PcjsActionWindow(document.all.link_update.href,\"\"); return false;\\'>".Translate("Mettre à jour")."</a></font><br>";?>');
<?php
  }
?>
...
<?
    ...
    echo "<td width=80>
          ...
          <span class=iewrap1>
          <span class=iewrap2 onClick=\"dropit2(dropmenu0);event.cancelBubble=true;return false\"><font face=$g_font size=$g_title_size><b><a class=head href=\"?a_action=alternate&a_archive=$p_archive\" onClick=\"if(document.layers) return dropit(event, 'document.dropmenu0')\">".Translate("Archive")."</a></b></font>
          </span>
          </span>
          </layer>
          </ilayer>
    ...
    echo "<td width=80>
          ...
          <layer visibility=show>
          <span class=iewrap1>
          <span class=iewrap2 onClick=\"dropit2(dropmenu1);event.cancelBubble=true;return false\"><font face=$g_font size=$g_title_size><b><a class=head href=\"?a_action=alternate&a_archive=$p_archive\" onClick=\"if(document.layers) return dropit(event, 'document.dropmenu1')\">".Translate("Fichiers")."</a></b></font>
          </span>
          </span>
          </layer>
          </ilayer>
    ...
    echo "<td width=80>
          ...
          <layer visibility=show>
          <span class=iewrap1>
          <span class=iewrap2 onClick=\"dropit2(dropmenu2);event.cancelBubble=true;return false\"><font face=$g_font size=$g_title_size><b><a class=head href=\"?a_action=alternate&a_archive=$p_archive\" onClick=\"if(document.layers) return dropit(event, 'document.dropmenu2')\">".Translate("Visualiser")."</a></b></font>
          </span>
          </span>
          </layer>
          </ilayer>
    ...
    echo "<td width=5>
          ...
          <layer visibility=show>
          <span class=iewrap1>
          <span class=iewrap2 onClick=\"dropit2(dropmenu3);event.cancelBubble=true;return false\"><font face=$g_font size=$g_title_size><b><a class=head href=\"?a_action=alternate&a_archive=$p_archive\" onClick=\"if(document.layers) return dropit(event, 'document.dropmenu3')\"> ? </a></b></font>
          </span>
          </span>
          </layer>
          </ilayer>
...
<?
  ...
  function AppPhpzipStart($p_message="")
  {
    ...
    echo "</tr>";
    echo "<tr bgcolor=$g_text_bg><td>";
    echo "<font face=$g_font color=$g_text_color><p>&nbsp;<br><div align=center>".Translate("Pour continuer vous devez soit")." <a class=text href=$v_url?a_action=new>".Translate("créer")."</a> ".Translate("une archive").", <a class=text href=$v_url?a_action=read>".Translate("ouvrir")."</a> ".Translate("une archive").", ".Translate("soit")." ".Translate("demander de l'")."<a class=text href=$v_url?a_action=help&a_topic=start>".Translate("aide")."</a></div></p></font>";
    echo "</td></tr>";
    echo "</table>";
  ...
  function AppPhpzipAskCreate($p_message="")
  {
    ...
    echo "<script language='javascript' src='script/pcsexplorer.js'></script>";
    ...
    echo "<td><input type=text name=a_dir size=40 maxlength=60 value='./'>";
    global $PATH_INFO;
    echo " <INPUT TYPE=button name=bt value='".Translate('Parcourir')." ...' onClick='PcjsOpenExplorer(\"pcsexplorer.php3\", \"forms.formulaire.a_dir.value\", \"type=dir\", \"calling_dir=".dirname($PATH_INFO)."\", \"start_dir=.\")'>";
    echo "</td></tr>";
  ...
  function AppPhpzipCreate($p_archive, $p_type, $p_startfile, $p_dir="")
  {
    ...
    // ----- Display HTML footer
    if ($p_message == "")
      $p_message = Translate("Vous pouvez remplacer l'archive existante.");
    AppPhpzipFooter($p_message);
    }
  ...
  function AppPhpzipListTar($p_archive, $v_list, $p_message="")
  {
    ...
    TrFctStart(__FILE__, __LINE__, "AppPhpzipListTar", "$p_archive, $p_list");
...
?>
<div id="Layer2" style="position:absolute; width:300px; height:232px; z-index:1; overflow: auto; left: 20px; top: 85px; background-color: #CCCCCC; layer-background-color: #CCCCCC; border: 1px none #CCCCCC">
<?
...
<?
  ...
  function AppPhpzipHelp($p_archive, $p_topic)
  {
    ...
    echo "<p><br><div align=center><font face=$g_font size=$g_text_size color=$g_text_color>".Translate("Le manuel d'utilisation est disponible en ligne")." : <a class=text href=http://www.phpconcept.net/phpzip-manuel.php3>PhpConcept</a></font></div>";
    echo "</p></td></tr></table>";
  ...
  function AppPhpzipAbout($p_archive)
  {
    ...
    echo "</p>";
    echo "<p><div align=center><font face=$g_font size=$g_text_size color=$g_text_color>".Translate("Auteur")." : <a class=text href=mailto:vincent@blavet.net>Vincent Blavet</a></font></div>";
    echo "</p>";
    ...
    echo "<td><font face=$g_font size=$g_text_size color=$g_text_color> <a class=text href='http://www.phpconcept.net/phpziplib-manuel.php3' target=_blank>PhpZipLib ".$g_phpzip_version."</a></font></td>";
    echo "</tr>";
    echo "<tr>";
    echo "<td>&nbsp;</td>";
    echo "<td><font face=$g_font size=$g_text_size color=$g_text_color> <a class=text href='http://www.phpconcept.net/pcltar-index.php3' target=_blank>PclTar ".$g_pcltar_version."</a></font></td>";
    echo "</tr>";
    echo "<tr>";
    echo "<td>&nbsp;</td>";
    ...
    echo "<br><br><font face=$g_font size=$g_text_size color=$g_text_color>".Translate("Gestion des menus dynamiques")." : 'Drop down menu link' © <a class=text href=http://www.dynamicdrive.com target=_blank>Dynamic Drive</a></font>";
...
?>
<div id="Layer2" style="position:absolute; width:300px; height:252px; z-index:1; overflow: auto; left: 20px; top: 75px; background-color: #CCCCCC; layer-background-color: #CCCCCC; border: 1px none #CCCCCC">
<?
        // ----- Display the list of extracted files
...
*/
    ...
    while (($v_file = readdir($v_dir)))
    {
      ...
      if ( (ereg("(^lang-)([[:alnum:]]+)(\.inc\.php3$)", $v_file, $v_items)) )
      {
    ...
    echo "<INPUT TYPE='TEXT' name='a_text_color' value=\"$g_text_color\" size='8' maxlength='15'>";
    echo "<img src='images/button-sel.jpg' border='0' width='16' height='18' hspace='0' vspace='0' align='absmiddle'  onClick=\"PcjsColorChooser(document.forms.a_config.a_text_color,'value')\">";
    echo ", ".Translate("Arrière plan")." : ";
    echo "<INPUT TYPE='TEXT' name='a_text_bg' value=\"$g_text_bg\" size='8' maxlength='15'>";
    echo "<img src='images/button-sel.jpg' border='0' width='16' height='18' hspace='0' vspace='0' align='absmiddle'  onClick=\"PcjsColorChooser(document.forms.a_config.a_text_bg,'value')\">";
    ...
    echo "<INPUT TYPE='TEXT' name='a_text_link' value=\"$g_text_link\" size='8' maxlength='15'>";
    echo "<img src='images/button-sel.jpg' border='0' width='16' height='18' hspace='0' vspace='0' align='absmiddle'  onClick=\"PcjsColorChooser(document.forms.a_config.a_text_link,'value')\">";
    echo "</font>";
    echo "  </td><td width=10></td>";
    echo "</tr>";
    echo "<tr><td width=10></td><td>&nbsp</td><td width=5></td></tr>";
    ...
    echo "<INPUT TYPE='TEXT' name='a_title_color' value=\"$g_title_color\" size='8' maxlength='15'>";
    echo "<img src='images/button-sel.jpg' border='0' width='16' height='18' hspace='0' vspace='0' align='absmiddle'  onClick=\"PcjsColorChooser(document.forms.a_config.a_title_color,'value')\">";
    echo ", ".Translate("Arrière plan")." : ";
    echo "<INPUT TYPE='TEXT' name='a_title_bg' value=\"$g_title_bg\" size='8' maxlength='15'>";
    echo "<img src='images/button-sel.jpg' border='0' width='16' height='18' hspace='0' vspace='0' align='absmiddle'  onClick=\"PcjsColorChooser(document.forms.a_config.a_title_bg,'value')\">";
    echo "  </td><td width=10></td>";
    echo "</tr>";
    echo "<tr><td width=10></td><td>&nbsp</td><td width=5></td></tr>";
    echo "<tr>";
    ...
    echo "<INPUT TYPE='TEXT' name='a_error_color' value=\"$g_error_color\" size='8' maxlength='15'>";
    echo "<img src='images/button-sel.jpg' border='0' width='16' height='18' hspace='0' vspace='0' align='absmiddle'  onClick=\"PcjsColorChooser(document.forms.a_config.a_error_color,'value')\">";
    echo ", ".Translate("Arrière plan")." : ";
    echo "<INPUT TYPE='TEXT' name='a_error_bg' value=\"$g_error_bg\" size='8' maxlength='15'>";
    echo "<img src='images/button-sel.jpg' border='0' width='16' height='18' hspace='0' vspace='0' align='absmiddle'  onClick=\"PcjsColorChooser(document.forms.a_config.a_error_bg,'value')\">";
    echo "  </td><td width=10></td>";
    echo "</tr>";
    echo "<tr><td width=10></td><td>&nbsp</td><td width=10></td></tr>";
...
?>
<div id="Layer2" style="position:absolute; width:300px; height:200px; z-index:1; overflow: auto; left: 20px; top: 60px; background-color: #CCCCCC; layer-background-color: #CCCCCC; border: 1px none #CCCCCC">
<?
        // ----- Display the list of extracted files
...
<?
        ...
        echo "<input type=text name=a_path size=15 maxlength=60 value=./>";
        ...
        // TBC
        global $PATH_INFO;
        echo " <INPUT TYPE=button name=bt value='".Translate('Parcourir')." ...' onClick='PcjsOpenExplorer(\"pcsexplorer.php3\", \"forms.formulaire.a_path.value\", \"type=dir\", \"calling_dir=".dirname($PATH_INFO)."\", \"start_dir=.\")'>";
  ...
  TrFctMessage(__FILE__, __LINE__, 2, "action='$a_action'");

  $v_message_window = 0;

  // ----- Check for no $a_archive argument
  if (!isset($a_archive))
    $a_archive = "";
  ...
  switch ($a_action) {
    ...
    case "new_do" :
      AppPhpzipCreate($a_archive, $a_type, $a_startfile, $a_dir);
    break;

    case "new_replace" :
      if ($a_submit==Translate("Oui"))
        AppPhpzipReplace($a_archive, $a_type, $a_startfile);
      else
        AppPhpzipAskCreate();
    break;
    ...
    case "add" :
      if ($a_archive == "")
        ...
        AppPhpzipExplorer($a_archive, "add", $a_dir);
    break;
    ...
    case "add_list" :
      ...
      {
        AppPhpzipAddList($a_archive, $a_file, $a_type, $a_dir, $a_stored_dir);
      }
    break;
    ...
    case "update" :
      if ($a_archive == "")
        ...
        AppPhpzipExplorer($a_archive, "update", $a_dir);
    break;
    ...
    case "update_list" :
      ...
      {
        AppPhpzipUpdate($a_archive, $a_file, $a_type, $a_dir, $a_stored_dir);
      }
    break;
    ...
    case "update_list_do" :
      ...
      {
        AppPhpzipUpdateList($a_archive, $a_file, $a_type, $a_dir, $a_stored_dir);
        $v_message_window = 1;
      }
    break;
    ...
    case "download" :
      if ($a_archive == "")
        ...
        AppPhpzipDownload($a_archive);
    break;

    case "erase_do" :
      if ($a_submit == Translate("Supprimer"))
        AppPhpzipErase($a_archive);
      else
        AppPhpzipList($a_archive);
    break;
    ...
    case "unzip_do" :
      if ($a_archive == "")
        ...
        if ((isset($a_extractall)) && ($a_extractall))
          AppPhpzipUnzip($a_archive, $a_path, $a_remove_dir);
        else
        {
          AppPhpzipUnzipList($a_archive, $a_file, $a_path, $a_remove_dir);
        }
    break;
    ...
    case "del_file" :
      if ($a_archive == "")
        ...
        AppPhpzipAskDeleteFile($a_archive);
    break;
    ...
    case "del_file_do" :
      ...
      {
        if (AppPhpzipArchiveType($a_archive)=="zip")
          AppPhpzipDeleteFileByIndex($a_archive, $a_index);
        else
          AppPhpzipDeleteFileList($a_archive, $a_file);
        $v_message_window = 1;
      }
    break;
    ...
    case "help" :
        AppPhpzipHelp($a_archive, $a_topic);
    break;

    case "about" :
        AppPhpzipAbout($a_archive);
    break;

    case "option" :
      AppPhpzipConfiguration($a_archive);
    break;

    case "option_do" :
      AppPhpzipConfigurationChange($a_archive, $a_lang, $a_font, $a_header, $a_footer, $a_trace, $a_trace_level, $a_trace_mode, $a_trace_filename,
                                   $a_text_bg, $a_text_color, $a_text_link, $a_title_bg, $a_title_color, $a_error_bg, $a_error_color,
                                   $a_text_size, $a_title_size, $a_subtitle_size, $a_error_size, $a_footer_size, $a_home_dir, $a_view_archive);
    break;

    case "alternate" :
      AppPhpzipAlternateMenu($a_archive);
    break;

    case "extract_index_ask" :
      AppPhpzipExtractByIndexAsk($a_archive, $a_index);
      $v_message_window = 1;
    break;

    case "extract_index" :
      AppPhpzipExtractByIndex($a_archive, $a_index, $a_path, $a_remove_dir);
      $v_message_window = 1;
    break;
    ...
    case "update_index" :
      ...
      {
        AppPhpzipUpdate($a_archive, $a_file);
        $v_message_window = 1;
      }
    break;

    case "test" :
      AppPhpzipActionHeader($a_archive);
  ...
  // ----- Specific code for javascript dynamic menu
  if (!$v_message_window)
  {
  echo "\n";
  echo "<div id=dropmenu0 style=\"position:absolute;left:0;top:0;layer-background-color:$g_title_bg;background-color:$g_title_bg;width:120;visibility:hidden;border:1px solid black;padding:0px\">\n";
?>
<script language="JavaScript1.2">
if (document.all)
...
<?
  echo "<div id=dropmenu1 style=\"position:absolute;left:0;top:0;layer-background-color:$g_title_bg;background-color:$g_title_bg;width:120;visibility:hidden;border:1px solid black;padding:0px\">\n";
?>
...
<?
  echo "<div id=dropmenu2 style=\"position:absolute;left:0;top:0;layer-background-color:$g_title_bg;background-color:$g_title_bg;width:120;visibility:hidden;border:1px solid black;padding:0px\">\n";
?>
<script language="JavaScript1.2">
if (document.all)
...
<?
  echo "<div id=dropmenu3 style=\"position:absolute;left:0;top:0;layer-background-color:$g_title_bg;background-color:$g_title_bg;width:120;visibility:hidden;border:1px solid black;padding:0px\">\n";
?>
<script language="JavaScript1.2">
if (document.all)
...
document.dropmenu3.onclick=hidemenu3
}
</script>
